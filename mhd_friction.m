...
%    title(['v_x and v_y and b_y at t =' num2str(t)]);
    ...
    subplotrc(4,1,4,1)
    pmax_plot= max([ pmax_plot ; abs(p2) ]);
    plot(x,p2)
    xlim([ x(1) x(ni) ])
    if( pmax_plot>0 ) 
        ylim([ -pmax_plot pmax_plot ])
    end
    xlabel('x')
    ylabel('Pressure')
    pause(tpause)
...
% First, advance quatities 1/2 time step (predictor step).
  for i= 2:ni-1
    ...
    p1(i)= 0.5*(p1(i)+p2(i))  - dt*( p2(i+1)*vx2(i+1) - p2(i-1)*vx2(i-1))/dx2 + (gamma-1)*dt*(vx2(i+1)-vx2(i-1))/dx2*p2(i) ;
    vx1(i)= 0.5*(vx1(i)+vx2(i)) + dt*(...                        % Note "..." for continuation
          -vx2(i)*( vx2(i+1) - vx2(i-1) )/dx2...
          +( -( p2(i+1) -  p2(i-1) )/dx2 -cz(i)*by2(i) )/d2(i)     );
...
%  the d2 and by2 equations be evaluated after the vx2 equation. 
  for i= 2:ni-1
    vy2(i)= vy1(i) + dt*( ...
                         -vx2(i)*( vy2(i+1) - vy2(i-1) )/dx2 ...
                            +cz(i)*bx0/d2(i)                    );
    p2(i)=p1(i) - dt*( p2(i+1)*vx2(i+1) - p2(i-1)*vx2(i-1))/dx2 + (gamma-1)*dt*(vx2(i+1)-vx2(i-1))/dx2*p2(i) ;
    vx2(i)= vx1(i) + dt*( ...
               -vx2(i)*( vx2(i+1) - vx2(i-1) )/dx2 ...
      +( -( p2(i+1) - p2(i-1) )/dx2 -cz(i)*by2(i) )/d2(i)      );
